...
export interface Bill { id: string; shopId: string; billNumber: string; invoiceNumber?: string; amount: number; description?: string; paidAmount: number; pendingAmount: number; billDate: string; dueDate: string; reminderDate?: string; status: BillStatus; attachment?: string; remarks?: string; createdAt: string; shop?: { shopName: string; ownerName: string; phone: string; whatsapp?: string; }; payments?: Payment[]; }
...
export interface DashboardStats { totalShops: number; totalBills: number; overdueCount: number; pendingCount: number; paidCount: number; totalOutstanding: number; collectedThisMonth: number; weeklyDue: Bill[]; recentBills: Bill[]; }
